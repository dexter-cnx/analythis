...
export function inspectArchitecture(inventory: Inventory, profile: ProfileName): Blueprint['architecture'] {
  const style: string[] = [];
  const layers: string[] = [];
  const dependency_rules: string[] = [];
...
export function inspectModules(inventory: Inventory): BlueprintModule[] {
...
function describeModulePurpose(name: string): string {
  switch (name) {
...
function inferDependsOn(name: string): string[] {
  if (name === 'app') return ['lib', 'src'];
  if (name === 'features') return ['src', 'lib'];
  if (name === 'server') return ['api'];
  if (name === 'apps') return ['packages'];
  return [];
}
