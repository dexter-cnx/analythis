...
exports.inspectRisks = inspectRisks;
exports.inspectRefactorOpportunities = inspectRefactorOpportunities;
function inspectRisks(inventory, blueprintSoFar) {
    const risks = [];
    const modules = blueprintSoFar.modules ?? [];
    const architecture = blueprintSoFar.architecture;
    const deps = blueprintSoFar.dependencies ?? [];
    if (inventory.repoType === 'unknown')
        risks.push('Repository type could not be confidently inferred.');
    if (!inventory.testLocations.length)
        risks.push('No obvious test locations were detected.');
    if (!inventory.ciFiles.length)
        risks.push('No CI pipeline files were detected.');
    ...
    if (!blueprintSoFar.events?.length)
        risks.push('No eventing or job patterns were detected; async workflows may be tightly coupled.');
    return risks;
}
function inspectRefactorOpportunities(inventory, blueprintSoFar) {
    const opportunities = [];
